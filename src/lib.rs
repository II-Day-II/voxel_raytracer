...
const TRI_VERTICES: &[Vertex] = &[
    Vertex { position: [ 0.0,  0.5, 0.0], color: [1.0, 0.0, 0.0], tex_coords: [0.0, 0.0], },
    Vertex { position: [-0.5, -0.5, 0.0], color: [0.0, 1.0, 0.0], tex_coords: [0.0, 0.0],},
    Vertex { position: [ 0.5, -0.5, 0.0], color: [0.0, 0.0, 1.0], tex_coords: [0.0, 0.0],},
];
...
const _CUBE_VERTICES: &[Vertex] = &[
    ...
    Vertex { position: [-0.5,  0.5, 0.5], color: [1.0, 1.0, 0.0], tex_coords: [0.0, 0.0],}, 
    Vertex { position: [-0.5, -0.5, 0.5], color: [1.0, 0.0, 0.0], tex_coords: [0.0, 0.0],}, 
    Vertex { position: [ 0.5, -0.5, 0.5], color: [1.0, 0.0, 1.0], tex_coords: [0.0, 0.0],},
     
    Vertex { position: [ 0.5,  0.5,-0.5], color: [0.0, 1.0, 1.0], tex_coords: [0.0, 0.0],},
    Vertex { position: [-0.5,  0.5,-0.5], color: [0.0, 1.0, 0.0], tex_coords: [0.0, 0.0],},
    Vertex { position: [-0.5, -0.5,-0.5], color: [0.0, 0.0, 0.0], tex_coords: [0.0, 0.0],},
    Vertex { position: [ 0.5, -0.5,-0.5], color: [0.0, 0.0, 1.0], tex_coords: [0.0, 0.0],},
];
...
impl State {
    ...
    async fn new(window: Window) -> Self {
        ...
        let diffuse_bytes = include_bytes!("vineboom.png");
        let diffuse_texture = texture::Texture::from_bytes(&device, &queue, diffuse_bytes, "vineboom.png").unwrap();
        ...
        let complex_vertex_buffer = device.create_buffer_init(
            &wgpu::util::BufferInitDescriptor {
                ...
                contents: bytemuck::cast_slice(PENT_VERTICES),
        ...
        let complex_index_buffer = device.create_buffer_init(
            &wgpu::util::BufferInitDescriptor {
                ...
                contents: bytemuck::cast_slice(PENT_INDICES),
                usage: wgpu::BufferUsages::INDEX,
            }
        );
        let complex_num_indices = PENT_INDICES.len() as u32;

        let tri_vertex_buffer = device.create_buffer_init(
            &wgpu::util::BufferInitDescriptor {
                label: Some("Vertex buffer"),
        ...
        let tri_index_buffer = device.create_buffer_init(
            &wgpu::util::BufferInitDescriptor {
                label: Some("Index buffer"),
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        match event {
            WindowEvent::CursorMoved { position, .. } => {
                ...
                let tl = cgmath::vec3(1., 0., 0.);
                let tr = cgmath::vec3(0., 0., 1.);
                let bl = cgmath::vec3(0., 1., 0.);
                let br = cgmath::vec3(1., 1., 0.);
                let t = tl.lerp(tr, xt);
                let b = bl.lerp(br, xt);
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        { // scope drops render pass at the end, so we can call encoder.finish()
            ...
            let (verts, inds, num, pipeline) = if self.is_space_pressed {
                (&self.complex_vertex_buffer, &self.complex_index_buffer, self.complex_num_indices, &self.render_pipeline)
            } else {
                (&self.tri_vertex_buffer, &self.tri_index_buffer, self.tri_num_indices, &self.render_pipeline_color)
            };
            render_pass.set_pipeline(pipeline);
            render_pass.set_bind_group(0, &self.diffuse_bind_group, &[]);
            render_pass.set_vertex_buffer(0, verts.slice(..));
            render_pass.set_index_buffer(inds.slice(..), wgpu::IndexFormat::Uint16);
            render_pass.draw_indexed(0..num, 0, 0..1);
        }
...
pub async fn run() {
    ...
    let window = WindowBuilder::new().with_title("Learning WGPU").build(&event_loop).unwrap();
